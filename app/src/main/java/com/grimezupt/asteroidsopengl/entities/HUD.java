...
public class HUD extends GLEntity implements TimerListener {
    ...
    private static final float SUBTEXT_SIZE = 2f;
    private float TEXT_SIZE = 2f;
    private float BIGTEXT_SIZE = 3f;
    ...
    private void init() {
        ...
        _scoreText.setScale(TEXT_SIZE);
        _scoreText.setAlign(GLText.ALIGN_RIGHT);
        // init lives array
        _lifeArray = new LifeArray(World.WIDTH * 0.5f, HUD_TEXT_MARGIN);
        _lifeArray.setScale(TEXT_SIZE);
        _lifeArray.setAlign(LifeArray.ALIGN_CENTER);
        // etc.
        _bigText = new GLText( World.WIDTH * 0.5f, World.HEIGHT * 0.5f);
        _bigText.setScale(BIGTEXT_SIZE);
        _bigText.setAlign(GLText.ALIGN_CENTER);
        _subText = new GLText(World.WIDTH * 0.5f, World.HEIGHT * 0.5f + BIGTEXT_SIZE);
        _subText.setScale(SUBTEXT_SIZE);
